...
const BuyButton = props => {
  ...
  function clickToBuy(){
    props.setTicketToBook(props.ticketId);
    props.openWindowForm();
  }

  return (
    <Button onClick={clickToBuy} disabled={Number.parseInt(props.price) <= 0}>
      <Text>Купить</Text>
      <Text>за {Math.round((props.priceChild * props.ticketPeople[0] + props.priceAdult * props.ticketPeople[1]) * props.currency * 100) / 100}</Text>
    </Button>
  );
}
